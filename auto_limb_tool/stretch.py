...
# Creates distance-based stretch networks for standard and custom IK chains.
class StretchMixin(object):
    ...
    def build_ik_stretch(
        ...
        # Compare the live control distance with the original limb length.
        if len(ik_chain) < 3:
            cmds.warning(
    ...
    def build_custom_stretch(self, ik_chain, ik_ctrl, primary_axis):
        # Blend between the original scale and the calculated stretch ratio.
        if len(ik_chain) < 2:
